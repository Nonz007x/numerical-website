...
export interface RootAndPoints {
  root: number;
  points: Point[];
}
...
abstract class RootFinder {
  ...
  protected pointCollector: PointCollector = new PointCollector();
  protected fx: math.EvalFunction;
  protected tolerance: number;

  constructor(inputFx: string, tolerance: number) {
    this.fx = math.compile(inputFx)
    this.tolerance = tolerance;
  }

  public calcFunc(value: number,): number {
    return this.fx.evaluate({ x: value });
  }

  public abstract findRoot(): RootAndPoints | null;
}

export class BisectionMethod extends RootFinder {
  xl: number;
  xr: number;

  constructor(fn: string, xl: number, xr: number, tolerance: number) {
    super(fn, tolerance);
    this.xl = xl;
    this.xr = xr;
  }

  public findRoot(): RootAndPoints | null {
    let fxm: number;
    let fxr: number;
    let xm = (this.xl + this.xr) / 2.0;

    try {
      fxm = this.calcFunc(xm);
      fxr = this.calcFunc(this.xr);
    ...
    let iterations = 0;
    if (this.calcFunc(this.xl) * this.calcFunc(this.xr) >= 0) {
      console.log("Initial guess does not bracket the root.");
      return null;
    } else {
      while (Math.abs(fxm) > this.tolerance && iterations < maxIterations) {
        if (fxm * fxr < 0) {
          this.xl = xm;
        } else {
          this.xr = xm;
        ...
        this.pointCollector.addPoint(xm, fxm);
        xm = (this.xl + this.xr) / 2.0;
        fxm = this.calcFunc(xm);
        console.log(xm);
        iterations++;
      }
    }
    return { root: xm, points: this.pointCollector.getPoints() };
  }
}

export function bisection_method2(fn: string, tolerance: number, xl: number, xr: number): RootAndPoints | null {
...
export function graphical_method2(fn: string, tolerance: number, start: number, stop: number): RootAndPoints | null {
  if (Number.isNaN(tolerance) || tolerance == 0)
    return null;
  const f = math.compile(fn);
  const incrementer = tolerance;
